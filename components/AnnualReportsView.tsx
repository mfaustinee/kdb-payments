...
// Dynamic calculations for Comparative Annual FY Summary and Total Debt Accrued (ESS)
const getDynamicFYs = (): string[] => {
  ...
  const list: string[] = [];
  for (let y = latestFYStart; y >= startY; y--) {
    list.push(`${y}/${y + 1}`);
  }
  return list;
};

const getDynamicCYs = (): number[] => {
  const startY = 2022;
  const currentYear = new Date().getFullYear();
  const list: number[] = [];
  for (let y = currentYear; y >= startY; y--) {
    list.push(y);
  }
  return list;
};

export const AnnualReportsView: React.FC<AnnualReportsViewProps> = ({ clients, returns }) => {
  const [selectedComparativeFYs, setSelectedComparativeFYs] = useState<string[]>(() => getDynamicFYs());
  ...
  // Get all 12 months for selected Financial Year
  const getFYMonths = (startY: number, endY: number) => {
    return [
  ...
  const annualCumulativeDebt = (() => {
    ...
    const lastFYMonth = fyMonths[fyMonths.length - 1];
    const lastFYMonthIndex = monthsList.indexOf(lastFYMonth.name);
    const lastFYMonthYear = lastFYMonth.year;
    return returns.filter(r => {
      const returnMonthIndex = monthsList.indexOf(r.period);
      const isPastOrSame = r.year < lastFYMonthYear || (r.year === lastFYMonthYear && returnMonthIndex <= lastFYMonthIndex);
  ...
  // Compile individual month details for the selected FY
  const compiledMonths = fyMonths.map(m => ({
    ...m,
  ...
  // Totals & Metrics for the Entire Year
  const annualGross = compiledMonths.reduce((sum, m) => sum + m.data.gross, 0);
  const annualDebt = compiledMonths.reduce((sum, m) => sum + m.data.debt, 0);
  const annualTotal = annualGross + annualDebt;
  const annualQty = compiledMonths.reduce((sum, m) => sum + m.data.totalQty, 0);
  const annualOutstanding = compiledMonths.reduce((sum, m) => sum + m.data.outstanding, 0);

  const allAnnualDebtClientIds = new Set(
    compiledMonths.flatMap(m => m.data.debtReturns.map(r => r.clientId))
  );
  const numAnnualDebtPayingDBOs = allAnnualDebtClientIds.size;

  const varianceCSL = annualTarget - annualGross;
  const varianceTotal = annualTarget - annualTotal;

  const rateExclDebt = annualTarget > 0 ? (annualGross / annualTarget) * 100 : 0;
  const rateInclDebt = annualTarget > 0 ? (annualTotal / annualTarget) * 100 : 0;
  ...
  // Active qualifying clients (using latest month of the financial year)
  const latestMonthName = fyMonths[11].name;
  const latestMonthYear = fyMonths[11].year;
  ...
  const proportionData = categoriesList.map(cat => {
    ...
    // Consistency Rate: Expected returns is sum of active count in each of the 12 months
  ...
  // DBO Leaderboard (Top 5 contributors in the FY)
  ...
  // Annual Pending/Outstanding Returns list
  ...
  const handleExportExcel = () => {
    ...
    exportAnnualReportToExcel({
      ...
      totalSubmissionRate,
      nonFilers: annualNonFilers,
      pendingReturns: mappedPending
    });
  };

  const getTargetForFY = (fy: string): number => {
    const stored = localStorage.getItem(`kdb_fy_target_${fy}`);
    ...
    return historical[fy] !== undefined ? historical[fy] : 2418192;
  };

  const getCollectionForFY = (fy: string): number => {
    if (fy === selectedFY) {
      return annualTotal;
    }
    
    // Check database returns for this FY
    const startY = parseInt(fy.split('/')[0], 10);
    const endY = parseInt(fy.split('/')[1], 10) || (startY + 1);
    const monthsInFY = getFYMonths(startY, endY);
    ...
    monthsInFY.forEach(m => {
      ...
      // Compute debt payments for this month
      const collMonthNum = (monthsList.indexOf(m.name) + 1) % 12;
      const matchYr = m.name === 'December' ? m.year + 1 : m.year;
      
      const dReturns = returns.filter(r => {
        const isPriorPeriod = r.year < m.year || 
          (r.year === m.year && monthsList.indexOf(r.period) < monthsList.indexOf(m.name));
        if (!isPriorPeriod) return false;
        if (!r.paymentDate) return false;
        ...
        if (isNaN(pDate.getTime())) {
          ...
          if (parts.length === 3) {
            ...
            const reportCollMonthNum = collMonthNum + 1;
            return mm === reportCollMonthNum && (yy === matchYr || yy === (matchYr % 100));
          }
          return false;
        }
        return (pDate.getMonth() + 1) === (collMonthNum + 1) && pDate.getFullYear() === matchYr;
    ...
    if (total === 0) {
      ...
      return historical[fy] !== undefined ? historical[fy] : 0;
    }
    return total;
  };

  const getOutstandingDebtForFY = (fy: string): number => {
    if (fy === selectedFY) {
      return annualOutstanding;
    }
    const startY = parseInt(fy.split('/')[0], 10);
    const endY = parseInt(fy.split('/')[1], 10) || (startY + 1);
    const monthsInFY = getFYMonths(startY, endY);
  ...
  const getOutstandingDebtForCY = (yr: number): number => {
    const cyReturns = returns.filter(r => r.year === yr);
    const total = cyReturns.filter(r => r.outstandingBalance > 0).reduce((sum, r) => sum + r.outstandingBalance, 0);
    ...
    if (total === 0) {
      ...
      return historical[yr] !== undefined ? historical[yr] : 0;
    }
    return total;
  };

  const comparativeFYs = getDynamicFYs().filter(fy => selectedComparativeFYs.includes(fy));
  const debtFYs = getDynamicFYs().filter(fy => fy !== '2021/2022');
  const debtCYs = getDynamicCYs();

  const totalFYDebt = debtFYs.reduce((sum, fy) => sum + getOutstandingDebtForFY(fy), 0);
  const totalCYDebt = debtCYs.reduce((sum, yr) => sum + getOutstandingDebtForCY(yr), 0);

  const maxRows = Math.max(debtFYs.length, debtCYs.length);
  const debtRows = [];
  for (let i = 0; i < maxRows; i++) {
    debtRows.push({
      fy: debtFYs[i] || '',
      fyAmount: debtFYs[i] ? getOutstandingDebtForFY(debtFYs[i]) : null,
      cy: debtCYs[i] || null,
      cyAmount: debtCYs[i] ? getOutstandingDebtForCY(debtCYs[i]) : null,
    });
  }

  // KPI Calculations
  const kpiRevenueCollected = getCollectionForFY(selectedFY);
  const kpiMaxAllowedDebt = kpiRevenueCollected * 0.05;
  const kpiTotalReceivable = totalFYDebt;
  const kpiActualDebtRatio = kpiRevenueCollected > 0 ? (kpiTotalReceivable / kpiRevenueCollected) * 100 : 0;
  const kpiStatus = kpiTotalReceivable <= kpiMaxAllowedDebt ? 'Within Limit' : 'Over Limit';
  const kpiEfficiency = kpiRevenueCollected > 0 ? ((kpiRevenueCollected - kpiTotalReceivable) / kpiRevenueCollected) * 100 : 100;

  return (
    <div className="space-y-6">
      {/* FILTER BAR FOR ANNUAL REPORT (Hidden on print) */}
      <div className="bg-slate-50/60 p-6 rounded-2xl border border-slate-100 grid grid-cols-1 md:grid-cols-4 gap-4 print:hidden">
      ...
      <div className="flex justify-end gap-3 print:hidden">
        ...
        >
          <Printer size={13} /> Export Annual PDF
        </button>
      </div>

      {/* PRINTABLE ANNUAL CONTAINER */}
      <div id="kdb-annual-report-printable" className="bg-white p-2 sm:p-4 rounded-3xl border border-slate-100 shadow-sm print:shadow-none print:border-none print:p-0 space-y-10 text-slate-800">
        
        {/* REPORT HEADER */}
        <div className="text-center space-y-2 border-b border-slate-200 pb-6">
          <h2 className="text-lg sm:text-xl font-black tracking-tight text-slate-950 uppercase">
            {selectedBranch} Annual Revenue Collection and Budget Execution Report
          </h2>
          <div className="flex justify-between items-center text-xs font-mono text-slate-500 max-w-lg mx-auto">
            ...
            <span>PERIOD: <strong className="text-slate-800">Full Year (FY {selectedFY})</strong></span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h3 className="font-black text-slate-900 text-sm tracking-tight uppercase flex items-center gap-2">
            ...
            I. Annual Executive Revenue Summary
          </h3>
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-200 font-semibold text-slate-700">
                <tr>
                  ...
                  <td className="p-4 text-right font-mono">{formatNumber(annualTarget)}</td>
                  <td className="p-4 text-right font-mono text-slate-800">{formatNumber(annualGross)}</td>
                ...
                <tr className="bg-slate-50/50">
                  ...
                  <td className="p-4 text-right font-mono text-emerald-600">{formatNumber(annualDebt)}</td>
                  <td className="p-4 text-right font-mono text-slate-300">-</td>
                ...
                <tr className="bg-slate-900 text-white font-black">
                  ...
                  <td className="p-4 text-right font-mono">{formatNumber(annualTarget)}</td>
                  <td className="p-4 text-right font-mono text-emerald-300">{formatNumber(annualTotal)}</td>
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-200 font-semibold text-slate-700">
                  ...
                  <tr className="bg-slate-900 text-white font-black">
                    ...
                    <td className="p-4 text-right font-mono">{formatNumber(annualTarget)}</td>
                    <td className="p-4 text-right font-mono">{formatNumber(annualGross)}</td>
                    <td className="p-4 text-right font-mono text-emerald-300">{formatNumber(annualDebt)}</td>
                    <td className="p-4 text-right font-mono font-bold bg-emerald-600 text-white">{formatNumber(annualTotal)}</td>
        ...
        <div className="space-y-6 pt-6 border-t border-slate-200">
          ...
          <div className="space-y-3">
            <h3 className="font-black text-slate-900 text-sm tracking-tight uppercase flex items-center gap-2">
              ...
              IV. Pending Payments & Outstanding Levies in Selected Financial Year
            </h3>
            ...
            {pendingReturns.length === 0 ? (
              <p className="p-8 bg-emerald-50 rounded-2xl border border-emerald-100 text-center text-emerald-600 text-xs font-black uppercase tracking-wide">
                No outstanding levies registered for this Financial Year!
              </p>
            ) : (
              <div className="overflow-x-auto border border-slate-200 rounded-2xl">
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-200 font-semibold text-slate-700">
                    ...
                    <tr className="bg-rose-50 font-black text-rose-950">
                      <td className="p-4 text-right uppercase text-[8px] tracking-wider" colSpan={4}>TOTAL OUTSTANDING FOR THE YEAR:</td>
                      <td className="p-4 text-right font-mono">{formatNumber(pendingReturns.reduce((sum, r) => sum + r.qty, 0))}</td>
                      <td className="p-4 text-right font-mono text-rose-700 text-sm">KES {formatNumber(annualOutstanding)}</td>
                      <td className="p-4"></td>
                    </tr>
                  </tbody>
                </table>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6 pt-4 border-t border-slate-100">
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 flex flex-col justify-between">
                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">FY Outstanding Balance</span>
                <span className="font-mono font-black text-sm text-slate-800 mt-2">KES {formatNumber(annualOutstanding)}</span>
                <span className="text-[8px] text-slate-400 font-bold uppercase mt-1">Branch Debt for Current Financial Year</span>
              </div>
              <div className="p-4 bg-rose-50 rounded-2xl border border-rose-100 flex flex-col justify-between">
                ...
                <span className="font-mono font-black text-sm text-rose-600 mt-2">KES {formatNumber(annualCumulativeDebt)}</span>
                <span className="text-[8px] text-rose-400 font-bold uppercase mt-1">Up to end of FY {selectedFY}</span>
              </div>
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-150 flex flex-col justify-between">
                ...
                <span className="font-mono font-black text-sm text-slate-800 mt-2">KES {formatNumber(annualOutstanding + 153375)}</span>
                <span className="text-[8px] text-slate-400 font-bold uppercase mt-1">Baseline + FY Arrears</span>
              </div>
            </div>
          </div>
        </div>

        {/* ADDITIONAL FY SUMMARY TABLES (Total Debt Accrued, KPI, and Comparative Annual FY Summary) */}
        <div className="space-y-6 pt-8 border-t border-slate-100">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* SUMMARY: Total Debt Accrued (ESS) */}
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4 print:shadow-none print:border-slate-300">
              <div className="flex justify-between items-center pb-2 border-b border-slate-100 print:border-b-2 print:border-slate-300">
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 bg-rose-600 rounded-full animate-pulse print:hidden"></span>
                  <h4 className="font-black text-slate-800 text-xs sm:text-sm uppercase tracking-wider">
                    SUMMARY: Total Debt Accrued (ESS)
                  </h4>
                </div>
                <button
                  onClick={() => setShowDebtSummary(!showDebtSummary)}
                  className="text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-all flex items-center gap-1 print:hidden"
                >
                  {showDebtSummary ? 'Collapse' : 'Expand'}
                </button>
              </div>

              {showDebtSummary && (
                <div className="overflow-x-auto border border-slate-300 rounded-xl print:rounded-none">
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-300 font-bold text-slate-800 text-[11px] uppercase tracking-wide">
                        <th className="p-3 border-r border-slate-300">Financial Year</th>
                        <th className="p-3 border-r border-slate-300 text-right">Amount (Ksh)</th>
                        <th className="p-3 border-r border-slate-300">Year</th>
                        <th className="p-3 text-right">Amount (Ksh)</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-300 font-bold text-slate-700">
                      {debtRows.map((row, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/50">
                          <td className="p-3 border-r border-slate-300 text-rose-800 font-bold font-sans">{row.fy}</td>
                          <td className="p-3 border-r border-slate-300 text-right font-mono text-slate-900">
                            {row.fyAmount !== null ? formatNumber(row.fyAmount) : '-'}
                          </td>
                          <td className="p-3 border-r border-slate-300 text-rose-800 font-bold font-sans">
                            {row.cy !== null ? row.cy : ''}
                          </td>
                          <td className="p-3 text-right font-mono text-slate-900">
                            {row.cyAmount !== null ? formatNumber(row.cyAmount) : '-'}
                          </td>
                        </tr>
                      ))}
                      <tr className="bg-slate-900 text-white font-black border-t-2 border-slate-950">
                        <td className="p-3 border-r border-slate-800 uppercase text-[10px] tracking-wider">TOTAL</td>
                        <td className="p-3 border-r border-slate-800 text-right font-mono text-emerald-300">{formatNumber(totalFYDebt)}</td>
                        <td className="p-3 border-r border-slate-800 uppercase text-[10px] tracking-wider">TOTAL</td>
                        <td className="p-3 text-right font-mono text-emerald-300">{formatNumber(totalCYDebt)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            {/* KPI: Total Receivables ≤ 5% of Revenue Collected */}
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4 print:shadow-none print:border-slate-300">
              <div className="flex justify-between items-center pb-2 border-b border-slate-100 print:border-b-2 print:border-slate-300">
                <div className="flex items-center gap-2">
                  <span className="w-2.5 h-2.5 bg-amber-500 rounded-full animate-pulse print:hidden"></span>
                  <h4 className="font-black text-slate-800 text-xs sm:text-sm uppercase tracking-wider">
                    KPI: Total Receivables ≤ 5% of Revenue Collected
                  </h4>
                </div>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <tbody className="divide-y divide-slate-300 font-bold text-slate-700">
                    <tr className="hover:bg-slate-50/50">
                      <td className="p-3 border-r border-slate-300 font-bold font-sans">Revenue Collected</td>
                      <td className="p-3 text-right font-mono text-slate-900">{formatNumber(kpiRevenueCollected)}</td>
                    </tr>
                    <tr className="hover:bg-slate-50/50">
                      <td className="p-3 border-r border-slate-300 font-bold font-sans">Max Allowed Debt (5%)</td>
                      <td className="p-3 text-right font-mono text-slate-900 font-bold">{formatNumber(kpiMaxAllowedDebt)}</td>
                    </tr>
                    <tr className="hover:bg-slate-50/50">
                      <td className="p-3 border-r border-slate-300 font-bold font-sans">Total Receivable (Debt)</td>
                      <td className="p-3 text-right font-mono text-slate-900">{formatNumber(kpiTotalReceivable)}</td>
                    </tr>
                    <tr className="hover:bg-slate-50/50">
                      <td className="p-3 border-r border-slate-300"></td>
                      <td className={`p-3 text-right font-mono font-black ${kpiTotalReceivable <= kpiMaxAllowedDebt ? 'text-emerald-600' : 'text-rose-600'}`}>
                        {kpiTotalReceivable <= kpiMaxAllowedDebt ? '-' : `(${formatNumber(kpiTotalReceivable - kpiMaxAllowedDebt)})`}
                      </td>
                    </tr>
                    <tr className="hover:bg-slate-50/50">
                      <td className="p-3 border-r border-slate-300 font-bold font-sans">Actual Debt Ratio</td>
                      <td className={`p-3 text-center font-mono text-white font-black ${kpiTotalReceivable <= kpiMaxAllowedDebt ? 'bg-emerald-600' : 'bg-red-600'}`}>
                        {kpiActualDebtRatio.toFixed(0)}%
                      </td>
                    </tr>
                    <tr className="hover:bg-slate-50/50">
                      <td className="p-3 border-r border-slate-300 font-bold font-sans">Status</td>
                      <td className={`p-3 text-center font-black uppercase tracking-wider text-[11px] ${kpiTotalReceivable <= kpiMaxAllowedDebt ? 'text-emerald-600 bg-emerald-50' : 'text-red-600 bg-red-50'}`}>
                        {kpiStatus}
                      </td>
                    </tr>
                    <tr className="bg-slate-900 text-white font-black border-t-2 border-slate-950">
                      <td className="p-3 border-r border-slate-800 uppercase text-[10px] tracking-wider">Collection Efficiency %</td>
                      <td className="p-3 text-right font-mono text-emerald-300">{kpiEfficiency.toFixed(2)}%</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          {/* COMPARATIVE ANNUAL FY SUMMARY */}
          <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4 print:shadow-none print:border-slate-300">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-100 print:border-b-2 print:border-slate-300">
              <div className="flex items-center gap-2">
                <span className="w-2.5 h-2.5 bg-indigo-600 rounded-full animate-pulse print:hidden"></span>
                <h4 className="font-black text-slate-800 text-xs sm:text-sm uppercase tracking-wider">
                  COMPARATIVE ANNUAL FY SUMMARY
                </h4>
              </div>
              {/* FY Selector Controls */}
              <div className="flex flex-wrap items-center gap-1.5 text-[10px] print:hidden">
                <span className="font-black text-slate-400 uppercase tracking-widest mr-1">Show FYs:</span>
                {getDynamicFYs().map(fy => {
                  const isChecked = selectedComparativeFYs.includes(fy);
                  return (
                    <button
                      key={fy}
                      type="button"
                      onClick={() => {
                        if (isChecked) {
                          setSelectedComparativeFYs(selectedComparativeFYs.filter(f => f !== fy));
                        } else {
                          setSelectedComparativeFYs([...selectedComparativeFYs, fy]);
                        }
                      }}
                      className={`px-2.5 py-1 rounded-lg text-[9px] font-bold tracking-wider uppercase transition-all border ${
                        isChecked
            ...
            <div className="overflow-x-auto border border-slate-300 rounded-xl print:rounded-none">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-300 font-bold text-slate-800 text-[11px] uppercase tracking-wide">
                    <th className="p-3 border-r border-slate-300">FY</th>
                    <th className="p-3 border-r border-slate-300 text-right">Target</th>
                    <th className="p-3 border-r border-slate-300 text-right">Coll Amount</th>
                    <th className="p-3 border-r border-slate-300 text-right">Variance</th>
                    <th className="p-3 text-right">Collection %</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-300 font-bold text-slate-700">
                  {comparativeFYs.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="p-8 text-center text-slate-400 italic font-medium">
                        No financial years selected. Select years using the buttons above to compare.
                      </td>
                    </tr>
                  ) : (
                    comparativeFYs.map(fy => {
                      const target = getTargetForFY(fy);
                      const collection = getCollectionForFY(fy);
                      const variance = target - collection;
                      const percent = target > 0 ? (collection / target) * 100 : 0;
                      return (
                        <tr key={fy} className="hover:bg-slate-50/50">
                          <td className="p-3 border-r border-slate-300 text-rose-800 font-bold font-sans">{fy}</td>
                          <td className="p-3 border-r border-slate-300 text-right font-mono text-slate-900">{formatNumber(target)}</td>
                          <td className="p-3 border-r border-slate-300 text-right font-mono text-slate-900">{formatNumber(collection)}</td>
                          <td className={`p-3 border-r border-slate-300 text-right font-mono ${variance < 0 ? 'text-blue-600' : 'text-rose-600'}`}>
                            {variance < 0 ? `(${formatNumber(Math.abs(variance))})` : formatNumber(variance)}
                          </td>
                          <td className="p-3 text-right font-mono text-slate-900">{percent.toFixed(2)}%</td>
                        </tr>
                      );
                    })
                  )}
